...
- Dark only. Tokens: pine `#14201B`, birch `#E9DFC7`, bronze `#B8863B`, mist `#8FA39A`, moss `#6A9C65`, ember `#A63A26`, wall `#1C2A24`. Font stack `"Iowan Old Style", "Palatino Linotype", "Book Antiqua", Palatino, Georgia, serif`. No external requests from the page.
...
- Sleep timer sentinel is the string `none`. Parameters `/valheim/panel/sleep-when-empty` (initial `false`, from config) and `/valheim/panel/empty-since` (initial `none`).
...
```
Real config: `panel: { enabled: true, sleepWhenEmpty: { enabledByDefault: false, idleMinutes: 60, checkEveryMinutes: 10 } },`.
...
3. Within 10 minutes with nobody online, `aws ssm get-parameter --name /valheim/panel/empty-since` shows an ISO timestamp (the timer runs even while the setting is off). Tick the box on the page and the countdown line appears; untick it again unless you want it on.
